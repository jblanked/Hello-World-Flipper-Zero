...
void HelloWorldApp::settings_item_selected_callback(void *context, uint32_t index)
{
    HelloWorldApp *app = (HelloWorldApp *)context;
    app->settingsItemSelected(index);
}

void HelloWorldApp::submenu_choices_callback(void *context, uint32_t index)
{
    HelloWorldApp *app = (HelloWorldApp *)context;
...
bool HelloWorldApp::save_char(const char *path_name, const char *value)
{
    Storage *storage = static_cast<Storage *>(furi_record_open(RECORD_STORAGE));
...
bool HelloWorldApp::load_char(const char *path_name, char *value, size_t value_size)
{
    Storage *storage = static_cast<Storage *>(furi_record_open(RECORD_STORAGE));
    File *file = storage_file_alloc(storage);
    char file_path[256];
...
HelloWorldApp::HelloWorldApp()
{
    ...
    if (!easy_flipper_set_submenu(&submenu, HelloWorldViewSubmenu,
                                  "Hello World", callback_exit_app, &viewDispatcher))
    {
        FURI_LOG_E(TAG, "Failed to allocate submenu");
        return;
    }

    submenu_add_item(submenu, "Run", HelloWorldSubmenuRun, submenu_choices_callback, this);
    submenu_add_item(submenu, "About", HelloWorldSubmenuAbout, submenu_choices_callback, this);
    submenu_add_item(submenu, "Settings", HelloWorldSubmenuSettings, submenu_choices_callback, this);
